...
def load_data_from_file(path):
    ...
    with open(path, "r", encoding="utf-8") as file:
        ...
        for line in lines:
            ...
            parts = line.split("\t")
            # Jetzt 17 Spalten (manpower entfernt):
            # ID,TAG,CLONG,RGB,LeaderTrait,LeaderName,T1,T2,T3,T4,T5,OwnedStates,StateCores,stability,war_support,factories,population
            if len(parts) < 17:
                parts += [""] * (17 - len(parts))
...
def update_clong():
    ...
    # Hole den neuen Wert aus dem Textfeld
    ...
    # Aktualisiere die Spalte "clong" des ausgewählten Eintrags
    nations_data[item_index]["clong"] = new_land

    # Aktualisiere die Listbox, um die Änderung anzuzeigen
    update_listbox()

    messagebox.showinfo("Erfolg", f"Die Spalte 'clong' wurde für das Land '{new_land}' aktualisiert.")


def save_data():
    # Nur für konsolidieren
...
def konsolidieren():
    ...
    updated_files.sort(key=lambda x: x[0])
    ...
    for num, fname in updated_files:
        if num != max_num:
            ...
            ts = get_file_timestamp(old_path)
            new_name = f"{ts}_snc.txt"
            os.rename(old_path, os.path.join(old_nations_dir, new_name))

    messagebox.showinfo("Erfolg", f"Konsolidierung abgeschlossen! sadge_nations.txt aktualisiert und alte updated-Dateien verschoben.")

def restart_script():
    os.execl(sys.executable, sys.executable, *sys.argv)

def save_or_update_entry():
    # Speichern (oben): nur interne Liste + history snapshot
...
button_save = tk.Button(frame_combined_button, text="Speichern", command=save_or_update_entry, bg="#444", fg="white", height=2)
button_save.grid(row=0, column=0, sticky="nsew")

button_land = tk.Button(frame_combined_button, text="Land", command=update_clong, bg="#444", fg="white", height=2)
...
# Nun 900 Höhe, Felder um 100 nach unten erweitert
